...
function setup() {
  ...
  computer = new Computer(width - 280,computerBase.body.position.y - 153,50,180);computerArcher = new ComputerArcher(width - 340,computerBase.body.position.y - 180,120,120);
...
function handleComputerArrowCollision() {
  ...
  for(var j = 0;j<computerArrows.length;j++){
    var cbaseCollision = Matter.SAT.collides(computerArrows[j].body,playerBase.body);

    var carcherCollision = Matter.SAT.collides(computerArrows[j].body,playerArcher.body);

    var playerCollision = Matter.SAT.collides(computerArrows[j].body,player.body);

    if(cbaseCollision.collided || carcherCollision.collided || playerCollision.collided){
